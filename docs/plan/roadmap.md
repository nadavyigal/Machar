...
| # | Step | State | Outcome |
|---|---|---|---|
| 1 | **Ministry fidelity check** on the 2026-27 data (Task 8 step 4) | ⬅ open, unticked | The dates are human-verified, not just internally consistent. This gates every other person using the app. The checklist now exists at `docs/fidelity-check-2026-2027.md` (generated by `node scripts/printSchoolYear.mjs`), so the task is bounded, but **not one box is ticked**. |
